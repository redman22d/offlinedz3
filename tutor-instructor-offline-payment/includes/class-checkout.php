...
class Checkout {
	...
	private static function get_items_in_checkout() {
		...
		// Tutor's CartModel::get_cart_items() returns raw stdClass DB rows, not
		// WP_Post instances, so normalise them the same way Tutor's own
		// CheckoutController::get_courses_data_by_order_items() does. Without
		// this, every item fails the `instanceof \WP_Post` check further down
		// and the cart appears empty even though it has items.
		$courses = array();

		foreach ( $cart['courses']['results'] as $item ) {
